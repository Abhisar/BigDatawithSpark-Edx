...
def wordCount(wordListRDD):
    ...
    """
    returnedRdd = wordListRDD.map(lambda x: (x,1)).reduceByKey(add)
...
def removePunctuation(text):
    ...
    return ' '.join([re.sub('[^A-Za-z0-9]+', '', i) for i in text.split()]).lower()    
...
shakespeareWordsRDD = shakespeareRDD.filter(isLineEmp).flatMap(lambda x : x.split())
...
# TODO: Replace <FILL IN> with appropriate code
shakeWordsRDD = shakespeareWordsRDD.filter(isLineEmp).flatMap(lambda x : x.split())
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = wordCount(shakeWordsRDD). takeOrdered (15, key=lambda (key,val): -val)
