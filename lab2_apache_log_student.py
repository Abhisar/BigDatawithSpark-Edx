...
sortedByDay = groupedByDay.mapValues(len).sortByKey()

avgDailyReqPerHost = (sortedByDay.join(dailyHosts)
                                 .map(lambda (x,y): (x , y[0] / y[1])).sortByKey()).cache()
...
badEndpointsTop20 = badEndpointsSum.takeOrdered (20, key=lambda (key,val): -val)
...
errHostsTop25 = errHostsSum.takeOrdered (25, key=lambda (key,val): -val)
...
errDateSorted = sc.parallelize(errDateSum.collect()).sortByKey().cache()
...
topErrDate = errDateSorted.takeOrdered (5, key=lambda (key,val): -val)
...
hourRecordsSorted = sc.parallelize(hourRecordsSum.collect()).sortByKey().cache()
